...
@section('page-title', 'Edit Anggota BPD')
@section('page-subtitle', 'Update data anggota BPD')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-md">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-bold text-gray-800">Edit Data Anggota BPD</h3>
            <p class="text-sm text-gray-500 mt-1">{{ $bpd->nama }} - {{ $bpd->jabatan }}</p>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.bpd.update', $bpd->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-1">
                    ...
                    <div class="mb-4">
                        ...
                        <img src="{{ asset('storage/' . $bpd->foto) }}" alt="{{ $bpd->nama }}" 
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-3">
                            {{ $bpd->foto ? 'Ganti Foto' : 'Upload Foto' }}
                        </label>
                ...
                <div class="md:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="jabatan" required
                            ...
                            <option value="Ketua BPD" {{ old('jabatan', $bpd->jabatan) == 'Ketua BPD' ? 'selected' : '' }}>Ketua BPD</option>
                            <option value="Wakil Ketua BPD" {{ old('jabatan', $bpd->jabatan) == 'Wakil Ketua BPD' ? 'selected' : '' }}>Wakil Ketua BPD</option>
                            <option value="Sekretaris BPD" {{ old('jabatan', $bpd->jabatan) == 'Sekretaris BPD' ? 'selected' : '' }}>Sekretaris BPD</option>
                            <option value="Anggota BPD" {{ old('jabatan', $bpd->jabatan) == 'Anggota BPD' ? 'selected' : '' }}>Anggota BPD</option>
                        </select>
                        @error('jabatan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" value="{{ old('nip', $bpd->nip) }}"
                    ...
                    <div>
                        ...
                        <select name="pendidikan"
                            ...
                            <option value="SD" {{ old('pendidikan', $bpd->pendidikan) == 'SD' ? 'selected' : '' }}>SD</option>
                            <option value="SMP" {{ old('pendidikan', $bpd->pendidikan) == 'SMP' ? 'selected' : '' }}>SMP</option>
                            <option value="SMA/SMK" {{ old('pendidikan', $bpd->pendidikan) == 'SMA/SMK' ? 'selected' : '' }}>SMA/SMK</option>
                            <option value="D3" {{ old('pendidikan', $bpd->pendidikan) == 'D3' ? 'selected' : '' }}>D3</option>
                            <option value="S1" {{ old('pendidikan', $bpd->pendidikan) == 'S1' ? 'selected' : '' }}>S1</option>
                            <option value="S2" {{ old('pendidikan', $bpd->pendidikan) == 'S2' ? 'selected' : '' }}>S2</option>
                            <option value="S3" {{ old('pendidikan', $bpd->pendidikan) == 'S3' ? 'selected' : '' }}>S3</option>
                        </select>
                        @error('pendidikan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="no_hp" value="{{ old('no_hp', $bpd->no_hp) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="urutan" value="{{ old('urutan', $bpd->urutan) }}" min="1"
                    ...
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="Aktif" {{ old('status', $bpd->status) == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="Tidak Aktif" {{ old('status', $bpd->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                    </div>
        ...
        <!-- Hidden Delete Form -->
        <form id="delete-form" action="{{ route('admin.bpd.destroy', $bpd->id) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
function confirmDelete() {
    if (confirm('Apakah Anda yakin ingin menghapus {{ $bpd->nama }} dari BPD?\n\nData yang sudah dihapus tidak dapat dikembalikan!')) {
